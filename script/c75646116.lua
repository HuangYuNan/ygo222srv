...
function c75646116.nacost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SendtoGrave(e:GetHandler(),128)
end
...
function c75646116.naop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.NegateAttack() then
		if Duel.Damage(1-tp,a:GetAttack(),64)~=0 then
...
function c75646116.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c75646116.cfilter,tp,12,0,1,nil) end
	Duel.Hint(3,tp,504)
	local g=Duel.SelectMatchingCard(tp,c75646116.cfilter,tp,12,0,1,1,nil)
...
function c75646116.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.SpecialSummon(c,0,tp,tp,false,false,5)
		if Duel.Damage(1-tp,900,64)~=0 then 
