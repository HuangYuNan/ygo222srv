...
function c10161005.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		if Duel.IsExistingTarget(c10161005.thfiter,tp,LOCATION_GRAVE+LOCATION_MZONE,0,1,e:GetLabelObject()) then sel=sel+2 end
		e:SetLabel(sel)
		return sel~=0
	end
	...
	if sel==1 then
		...
		local tg=Duel.SelectTarget(tp,c10161005.thfiter,tp,LOCATION_GRAVE+LOCATION_MZONE,0,1,1,e:GetLabelObject())
		Duel.SetOperationInfo(0,CATEGORY_TOHAND,tg,1,0,LOCATION_GRAVE+LOCATION_MZONE)
	end
end
