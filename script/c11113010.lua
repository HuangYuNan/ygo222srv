...
function c11113010.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(c11113010.filter,tp,LOCATION_DECK,0,1,nil) end
end
function c11113010.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetCondition(c11113010.setcon)
	e1:SetOperation(c11113010.setop)
	Duel.RegisterEffect(e1,tp)
end
function c11113010.setcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c11113010.filter,tp,LOCATION_DECK,0,1,nil)
end 
function c11113010.setop(e,tp,eg,ep,ev,re,r,rp)
    Duel.Hint(HINT_CARD,0,11113010)
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c11113010.filter,tp,LOCATION_DECK,0,1,1,nil)
...
function c11113010.thfilter(c)
	return c:IsSetCard(0x15c) and c:IsType(TYPE_TUNER) and not c:IsType(TYPE_PENDULUM) and not c:IsCode(11113010) and c:IsAbleToHand()
end
