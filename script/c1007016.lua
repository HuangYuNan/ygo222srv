...
function c1007016.initial_effect(c)
	--xyz summon
	aux.AddXyzProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0x245),3,2)
	c:EnableReviveLimit()
	--pendulum summon
	...
	e2:SetDescription(aux.Stringid(1855932,0))
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetCondition(c1007016.xycon)
	e2:SetTarget(c1007016.target)
	e2:SetOperation(c1007016.activate)
	c:RegisterEffect(e2)
	--spsummon
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(19990008,0))
...
function c1007016.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1007016.filter,tp,LOCATION_DECK,0,1,nil,tp) end
end
function c1007016.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(1007016,3))
	local tc=Duel.SelectMatchingCard(tp,c1007016.filter,tp,LOCATION_DECK,0,1,1,nil,tp):GetFirst()
...
function c1007016.dfilter(c)
	return c:IsDestructable()
end
function c1007016.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1007016.dfilter,tp,LOCATION_SZONE,LOCATION_SZONE,1,c) end
	local sg=Duel.GetMatchingGroup(c1007016.dfilter,tp,LOCATION_SZONE,LOCATION_SZONE,c)
