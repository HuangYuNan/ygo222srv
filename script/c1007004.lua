...
function c1007004.initial_effect(c)
	...
	e3:SetCountLimit(1,10703+EFFECT_COUNT_CODE_SINGLE)
	...
	e4:SetCountLimit(1,10704+EFFECT_COUNT_CODE_SINGLE)
	c:RegisterEffect(e4)
	--spsummon
	...
	e5:SetCountLimit(1,10703+EFFECT_COUNT_CODE_SINGLE)
	...
	e6:SetCountLimit(1,10704+EFFECT_COUNT_CODE_SINGLE)
	c:RegisterEffect(e6)
end
...
function c1007004.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,1007021,0x3245,0x4011,500,500,2,RACE_FIEND,ATTRIBUTE_DARK) end
...
function c1007004.spop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e)
		...
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,1007021,0x3245,0x4011,500,500,2,RACE_FIEND,ATTRIBUTE_DARK) then return end
...
function c1007004.spop1(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e)
		or Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,1007021,0x3245,0x4011,500,500,2,RACE_FIEND,ATTRIBUTE_DARK,1-tp) then return end
		local token=Duel.CreateToken(tp,1007021)
