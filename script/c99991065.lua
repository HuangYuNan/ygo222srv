...
function c99991065.initial_effect(c)
    aux.AddFusionProcFun2(c,aux.FilterBoolFunction(Card.IsSetCard,0x2e2),aux.FilterBoolFunction(c99991065.chfilter),true)
	c:EnableReviveLimit()
	--to deck
...
function c99991065.chfilter(c)
	return c:IsAttribute(ATTRIBUTE_WATER) or c:IsSetCard(0x62e0)
end
...
function c99991065.spfilter2(c)
	return  (c:IsAttribute(ATTRIBUTE_WATER) or c:IsSetCard(0x62e0)) and c:IsCanBeFusionMaterial() and c:IsAbleToGraveAsCost()
end
...
function c99991065.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return e:GetHandler():IsAbleToRemove() end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,e:GetHandler(),1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c99991065.op(e,tp,eg,ep,ev,re,r,rp)
    if e:GetHandler():IsRelateToEffect(e) and e:GetHandler():IsControler(tp) and Duel.Remove(e:GetHandler(),nil,REASON_EFFECT)~=0 then
         if  e:GetHandler():IsForbidden()  or Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not 
		 e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP) then
        Duel.SendtoGrave(tc,REASON_RULE)
    elseif Duel.SpecialSummon(e:GetHandler(),0,tp,tp,false,false,POS_FACEUP)>0  and e:GetHandler():IsFaceup() then
