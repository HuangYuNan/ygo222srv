...
function c1007006.detg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1007006.filter,tp,LOCATION_DECK,0,1,nil,tp) end
end
function c1007006.deop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(1007006,3))
	local tc=Duel.SelectMatchingCard(tp,c1007006.filter,tp,LOCATION_DECK,0,1,1,nil,tp):GetFirst()
...
function c1007006.cfilter(c,tp)
	return c:IsSetCard(0xa245) and c:IsPreviousLocation(LOCATION_ONFIELD) and c:GetPreviousControler()==tp
end
...
function c1007006.tkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	and re:GetHandler():IsSetCard(0x245)
