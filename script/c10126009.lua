...
function c10126009.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1,g2,tg1,tg2,tc=Duel.GetMatchingGroup(c10126009.eqfilter,tp,0,LOCATION_GRAVE,nil),Duel.GetFieldGroup(c10126009.eqfilter,tp,0,LOCATION_ONFIELD,nil)
...
function c10126009.eqtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return (Duel.GetLocationCount(1-tp,LOCATION_SZONE)>0 and Duel.IsExistingMatchingCard(nil,tp,0,LOCATION_ONFIELD,1,nil) or Duel.IsExistingMatchingCard(c10126009.eqfilter,tp,0,LOCATION_SZONE,1,nil)) end
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,nil,1,0,0)
end
function c10126009.eqfilter(c)
	return c:GetSequence()<5 and ((tc:IsType(TYPE_EQUIP) and not tc:CheckEquipTarget(c)) or not tc:IsType(TYPE_EQUIP))
end
...
function c10126009.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif ft==0 and g:FilterCount(c10126009.eqfilter,nil)>0 then
	  Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_EQUIP)
	  tc=g:FilterSelect(1-tp,c10126009.eqfilter,1,1,nil):GetFirst()
	end
