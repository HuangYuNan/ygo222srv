function c11200083.fremoveQinglan(c)
	local t=c:GetCode()
	return t>11200079 and t<11200087 and t~=11200083 and c:IsAbleToRemoveAsCost()
end
function c11200083.fremoveQinglanMon(c)
	local t=c:GetCode()
	return t>11200079 and t<11200083 and c:IsAbleToRemove()
end
function c11200083.fremoveQinglanSp(c)
	local t=c:GetCode()
	return t>11200083 and t<11200087 and c:IsAbleToRemove()
end
...
function c11200083.coa(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c11200083.fremoveQinglan,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,nil)end
	local g=Duel.SelectMatchingCard(tp,c11200083.fremoveQinglan,tp,LOCATION_HAND+LOCATION_ONFIELD,0,1,999,nil)
...
function c11200083.opa(e,tp)
	...
	if e:GetLabel()%2>0 then Duel.Remove(Duel.SelectMatchingCard(tp,c11200083.fremoveQinglanMon,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil),POS_FACEUP,REASON_EFFECT)end
	if e:GetLabel()>1 then Duel.Remove(Duel.SelectMatchingCard(tp,c11200083.fremoveQinglanSp,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil),POS_FACEUP,REASON_EFFECT)end
end
