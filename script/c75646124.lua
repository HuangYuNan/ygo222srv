...
function c75646124.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsFaceup,tp,4,4,1,nil) end
end
function c75646124.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(3,tp,514)
	local g=Duel.SelectMatchingCard(tp,Card.IsFaceup,tp,4,4,1,10,nil)
	if g:GetCount()>0 then
		local tc=g:GetFirst() 
		while tc do
			if not tc:IsStatus(STATUS_SPSUMMON_TURN) then
			tc:SetStatus(STATUS_SPSUMMON_TURN,true) end
			Duel.SpecialSummon(tc,0,tp,tp,false,false,5)
