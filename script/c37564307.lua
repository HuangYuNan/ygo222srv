...
function c37564307.sppop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (g:GetCount()==0
		...
		while tc do
			...
			e1:SetReset(RESET_EVENT+0x1fc0000+EVENT_PHASE+PHASE_END)
			tc:RegisterEffect(e1,true)
			local e2=Effect.CreateEffect(e:GetHandler())
			e2:SetType(EFFECT_TYPE_SINGLE)
			e2:SetCode(EFFECT_DISABLE_EFFECT)
			e2:SetReset(RESET_EVENT+0x1fc0000+EVENT_PHASE+PHASE_END)
			tc:RegisterEffect(e2,true)
			local e3=Effect.CreateEffect(e:GetHandler())
			e3:SetType(EFFECT_TYPE_SINGLE)
			e3:SetCode(EFFECT_CANNOT_ATTACK)
			e3:SetReset(RESET_EVENT+0x1fc0000+EVENT_PHASE+PHASE_END)
