...
function c101116235.initial_effect(c)
	--pendulum summon
	...
	e2:SetDescription(aux.Stringid(101116235,0))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_PZONE)
	e2:SetTarget(c101116235.sptg)
	e2:SetOperation(c101116235.spop)
	c:RegisterEffect(e2)
	--setspell
	...
	e3:SetTarget(c101116235.target)
	e3:SetOperation(c101116235.op)
	c:RegisterEffect(e3)
end

function c101116235.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101116235.spop(e,tp,eg,ep,ev,re,r,rp)
...
function c101116235.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local b1=Duel.CheckLocation(tp,LOCATION_SZONE,6) or Duel.CheckLocation(tp,LOCATION_SZONE,7)
	if chk==0 then return b1
		and Duel.IsExistingMatchingCard(c101116235.filter,tp,LOCATION_DECK,0,1,nil) end
end
function c101116235.filter(c)
	return isFletcher(c) and not c:IsCode(101116235) and c:IsType(TYPE_PENDULUM) and not c:IsForbidden()
end
function c101116235.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local b1=2
	...
	local g1=Duel.GetMatchingGroup(c101116235.filter,tp,LOCATION_DECK,0,nil)
