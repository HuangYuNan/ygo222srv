...
function c66612307.limit(e,se,sp,st)
...
function c66612307.spfilter1(c,tp)
	return c:IsSetCard(0x660) and c:IsType(TYPE_MONSTER)  and c:IsAbleToRemoveAsCost() and c:IsCanBeFusionMaterial(nil,true)
		and Duel.IsExistingMatchingCard(c66612307.spfilter2,tp,LOCATION_MZONE,0,1,c)
end
function c66612307.spfilter2(c)
	return (c:GetLevel()==1 or c:GetLevel()==5) and c:IsType(TYPE_MONSTER) and c:IsCanBeFusionMaterial(nil,true) and c:IsAbleToRemoveAsCost()
end
function c66612307.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-2
		and Duel.IsExistingMatchingCard(c66612307.spfilter1,tp,LOCATION_MZONE,0,1,nil,tp)
end
function c66612307.sprop(e,tp,eg,ep,ev,re,r,rp,c)
    Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(66612307,0))
	local g1=Duel.SelectMatchingCard(tp,c66612307.spfilter1,tp,LOCATION_MZONE,0,1,1,nil,tp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(66612307,1))
	local g2=Duel.SelectMatchingCard(tp,c66612307.spfilter2,tp,LOCATION_MZONE,0,1,1,g1:GetFirst())
	g1:Merge(g2)
