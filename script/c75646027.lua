...
function c75646027.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c75646027.cfilte,tp,12,0,1,e:GetHandler()) end
	local g=Duel.SelectMatchingCard(tp,c75646027.cfilte,tp,12,0,1,1,e:GetHandler())
	Duel.Release(g,0x80)
end
...
function c75646027.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c75646027.cfilter,tp,4,0,1,nil)
end
function c75646027.filter(c)
	return c:IsFaceup() and c:IsControlerCanBeChanged()
end
function c75646027.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(4) and chkc:IsControler(1-tp) and c75646027.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c75646027.filter,tp,0,4,1,nil) end
	Duel.Hint(3,tp,518)
	local g=Duel.SelectTarget(tp,c75646027.filter,tp,0,4,1,1,nil)
...
function c75646027.rfilter(c)
	return c:IsType(1) and c:IsReleasableByEffect()
end
