...
function cm.filter0(c)
	return c:IsCanBeFusionMaterial() and c:IsFaceup() and c:IsAbleToDeck()
end
function cm.filter1(c,e)
	return c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e) and c:IsFaceup() and c:IsAbleToDeck()
end
function cm.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and c:CheckFusionMaterial(m,nil,chkf) and c:IsHasEffect(37564800)
end
