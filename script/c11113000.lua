...
function c11113000.filter0(c)
	return (c:IsFaceup() or c:IsLocation(LOCATION_MZONE+LOCATION_GRAVE)) and c:IsCanBeFusionMaterial() 
	    and c:IsAbleToRemove()
end
function c11113000.filter1(c,e)
	return (c:IsFaceup() or c:IsLocation(LOCATION_MZONE+LOCATION_GRAVE)) and c:IsCanBeFusionMaterial() 
...
function c11113000.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg1=Duel.GetMatchingGroup(c11113000.filter0,tp,LOCATION_MZONE+LOCATION_GRAVE+LOCATION_EXTRA,0,nil)
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c11113000.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c11113000.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
	local mg1=Duel.GetMatchingGroup(c11113000.filter1,tp,LOCATION_GRAVE+LOCATION_MZONE+LOCATION_EXTRA,0,nil,e)
	...
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c11113000.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end
