...
function cm.sprfilter1(c,tp,fc)
	return c:IsCode(37564765) and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial(fc)
		and Duel.IsExistingMatchingCard(cm.sprfilter2,tp,LOCATION_MZONE,0,1,c,fc)
end
function cm.sprfilter2(c,fc)
	return c:IsRace(RACE_FAIRY) and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial(fc)
end
