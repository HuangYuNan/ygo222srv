...
function c5012607.initial_effect(c)
	...
	e4:SetCountLimit(1,5012607+EFFECT_COUNT_CODE_DUEL)
	c:RegisterEffect(e4)
	--get
...
function c5012607.refilter(c)
	return (c:IsSetCard(0x350) or c:IsSetCard(0x35a)) and c:IsAbleToRemoveAsCost()
end
...
function c5012607.aclimit(e,re,tp)
	return not (re:GetHandler():IsSetCard(0x350) or re:GetHandler():IsSetCard(0x35a))
end
...
function c5012607.cfilter(c)
	return c:IsFaceup() and (c:IsSetCard(0x350) or c:IsSetCard(0x35a))
end
...
function c5012607.spfilter(c,e,tp)
	return c:IsCanBeSpecialSummoned(e,0,tp,true,true) and c:IsSetCard(0x35a) and c:IsType(TYPE_MONSTER)
end
function c5012607.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	and  Duel.IsExistingMatchingCard(c5012607.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,0)
end
function c5012607.spfilter2(c,e,tp)
	return c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SYNCHRO,tp,false,false) and c:IsCode(5012604)
end
function c5012607.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c5012607.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA,0,1,1,nil,e,tp)
	if g:GetCount()>0 and Duel.SpecialSummon(g,0,tp,tp,true,true,POS_FACEUP)>0 and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 and
	Duel.IsExistingMatchingCard(c5012607.spfilter2,tp,LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA,0,1,nil,e,tp) and
