...
function c29201208.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(c29201208.costfilter,tp,LOCATION_HAND,0,1,e:GetHandler()) 
      and not e:GetHandler():IsPublic() end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
    local g=Duel.SelectMatchingCard(tp,c29201208.costfilter,tp,LOCATION_HAND,0,1,1,e:GetHandler())
    local tc=g:GetFirst()
