...
function c75646118.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(4) and c75646118.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c75646118.filter,tp,0,4,1,nil) end
	Duel.Hint(3,tp,514)
	local g=Duel.SelectTarget(tp,c75646118.filter,tp,0,4,1,1,nil)
...
function c75646118.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(c75646118.filter2,tp,16,0,1,nil) end
	Duel.SetOperationInfo(0,8,nil,1,tp,16)
end
function c75646118.op(e,tp,eg,ep,ev,re,r,rp,chk)
	Duel.Hint(3,tp,506)
	local sg=Duel.SelectMatchingCard(tp,c75646118.filter2,tp,16,0,1,1,nil)
