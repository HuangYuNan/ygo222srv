...
function cm.effect_operation_3L(c,ec,chk)
	local ex1=Effect.CreateEffect(ec)
	...
	ex2:SetReset(senya.lres(chk))
	c:RegisterEffect(ex2)
end
function cm.filter(c,e,tp)
	return c:IsType(TYPE_FUSION) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsHasEffect(37564800)
end
