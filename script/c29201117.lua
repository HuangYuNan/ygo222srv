...
function c29201117.initial_effect(c)
    ...
    e10:SetDescription(aux.Stringid(29201117,0))
    e10:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
    e10:SetProperty(EFFECT_FLAG_CARD_TARGET)
    e10:SetCode(EVENT_ATTACK_ANNOUNCE)
    e10:SetCountLimit(1)
    e10:SetTarget(c29201117.target)
    e10:SetOperation(c29201117.operation)
    c:RegisterEffect(e10)
end
function c29201117.filter8(c)
    return c:IsFaceup()
end
function c29201117.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chkc then return chkc:IsLocation(LOCATION_MZONE) and c29201117.filter8(chkc) end
    if chk==0 then return Duel.IsExistingTarget(c29201117.filter8,tp,LOCATION_MZONE,0,1,nil) end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
    Duel.SelectTarget(tp,c29201117.filter8,tp,LOCATION_MZONE,0,1,1,nil)
end
function c29201117.thop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if tc:IsFaceup() and tc:IsRelateToEffect(e) then
        ...
        e3:SetValue(c29201117.efilter)
        e3:SetReset(RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END)
        tc:RegisterEffect(e3)
    end
end
function c29201117.efilter(e,te)
